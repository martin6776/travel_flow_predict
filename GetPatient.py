...
class GetPatient:
    ...
    def get_patient_df(self, dt_start = '2020-01-01'):
        ...
        for i in range(len(messages)):
            value = (messages[i].get('provinceName'),messages[i].get('provinceShortName'),messages[i].get('currentConfirmedCount'), \
                     messages[i].get('confirmedCount'),messages[i].get('suspectedCount'),messages[i].get('curedCount'),messages[i].get('deadCount'), \
                     messages[i].get('locationId'),messages[i].get('statisticsData'))
            value_list.append(value)
            city_value = messages[i].get('cities')

            for j in range(len(city_value)):
                city_value_list = (city_value[j].get('cityName'),city_value[j].get('currentConfirmedCount'), \
                                   city_value[j].get('confirmedCount'),city_value[j].get('suspectedCount'),city_value[j].get('curedCount'),city_value[j].get('deadCount'), \
                                   city_value[j].get('locationId'),city_value[j].get('statisticsData'))
                city_list.append(city_value_list)

        df = pd.DataFrame(value_list, columns=['provinceName','provinceShortName','currentConfirmedCount','confirmedCount','suspectedCount','curedCount','deadCount','locationId','statisticsData'])
...
if __name__ == '__main__':
    ...
    print(patient.get_patient_df(yesterday))